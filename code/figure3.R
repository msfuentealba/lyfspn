...
results <- read_rds("./output/epigenetic_age_acceleration.rds") %>% filter(baseline==1&timepoint==2)
biomarker <- read_rds("./output/clinical_baseline.rds")
biomarker$feature[biomarker$feature=="EGFR"] <- "eGFR"
protein <- read_rds("./output/proteomics_baseline.rds")
metabo <- read_rds("./output/metabolomics_baseline.rds")
metabo$feature[metabo$feature=="Creatinine"] <- "Creatinine (M)"
lipid <- read_rds("./output/lipidomics_baseline.rds")
glycan <- read_rds("./output/glycomics_baseline.rds")
cell <- read_rds("./output/cytomics_baseline.rds")
...
correlations_top <- combined %>% filter(omics=="Clinical"&group!="Sham") %>% group_by(group,omics,patient,feature) %>% summarise(dif = mean(age_acc_difference), value = mean(value)) 
significant <- correlations_top %>% group_by(group,feature) %>% summarise(cor = cor(dif,value), p = pval_permutation(dif,value)) %>% na.omit
significant$direction <- ifelse(sign(significant$cor)==1,"Positive","Negative")
top_selected <- significant %>% group_by(group,direction) %>% summarise(feature = feature[which.min(p)]) 
correlations_top <- top_selected %>% left_join(correlations_top)
correlations_top$group <- factor(correlations_top$group, levels = c("TPE + IVIG\n(B)","TPE\n(B)","TPE\n(M)"))
correlations_top <- correlations_top %>% left_join(significant %>% dplyr::select(feature,direction,cor,p))
...
p1 <- ggplot(correlations_top %>% na.omit, aes(dif,value))+
  ...
  stat_correlation(data = correlations_top %>% filter(direction=="Negative"), label.x = 0.05, label.y = 0.2, method = "pearson", aes(label = paste0("R == ", round(cor,2))))+
  stat_correlation(data = correlations_top %>% filter(direction=="Negative"), label.x = 0.05, label.y = 0.05, method = "pearson", aes(label = paste0("p == ", p)))+
  stat_correlation(data = correlations_top %>% filter(direction=="Positive"), label.x = 0.95, label.y = 0.2, method = "pearson", aes(label = paste0("R == ", round(cor,2))))+
  stat_correlation(data = correlations_top %>% filter(direction=="Positive"), label.x = 0.95, label.y = 0.05, method = "pearson", aes(label = paste0("p == ", p)))+
