...
pairwise <- pairwise %>% left_join(pairwise %>% dplyr::group_by(group) %>% summarise(cor = cor(`Time point 2 vs 1`,`Time point 3 vs 2`), p = cor.test(`Time point 2 vs 1`,`Time point 3 vs 2`)$p.value))
...
p3 <- ggplot(pairwise, aes(`Time point 2 vs 1`,`Time point 3 vs 2`))+
  ...
  stat_correlation(label.x = 0.05, label.y = 0.93, method = "pearson", aes(label = paste0("R == ", round(cor,2)))) +
  stat_correlation(label.x = 0.05, label.y = 0.83, method = "pearson", aes(label = paste0("p = ", p)), parse = FALSE)+
