...
iage <- read_rds("./data/iage_diff.rds")
...
writexl::write_xlsx(compare_sham %>% set_names("omics", "mean_r", "group", "feature", "fdr_zero", "fdr_sham"), path = "./output/tables/Sup_Table3.xlsx")
keep <- compare_sham %>%
...
correlations_top <- combined %>%
  filter(group == "TPE + IVIG\n(B)" & feature %in% top$feature) %>%
  group_by(omics, patient, feature) %>%
  summarise(dif = mean(age_acc_difference), value = mean(value))
...
correlations_top <- correlations_top %>% left_join(correlations_top %>% group_by(direction, omics) %>% summarise(cor = cor(dif, value, method = "spearman")))

correlations_top <- correlations_top %>%
  group_by(direction, omics) %>%
  mutate(correlation_label = paste("R = ", round(cor, 2)))
...
cell.plot(
  x = setNames(-log10(enrich_result$p.adjust), enrich_result$Description),
...
plot_grid(plot_grid(plot_grid(a, labels = c("a")),
  ...
  rel_widths = c(1, 0.7)
), plot_grid(d, NULL, labels = c("d"), rel_widths = c(1, 0.01)), nrow = 2, rel_heights = c(1, 0.5))
dev.off()
