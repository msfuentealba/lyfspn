...
protein <- read_rds("./output/proteomics_difference.rds")
metabo <- read_rds("./output/metabolomics_difference.rds")
lipid <- read_rds("./output/lipidomics_difference.rds")
glycan <- read_rds("./output/glycomics_difference.rds")
cell <- read_rds("./output/cytomics_difference.rds")
...
correlations_top <- combined %>% filter(group=="TPE + IVIG\n(B)"&feature%in%keep) %>% group_by(omics,patient,feature) %>% summarise(dif = mean(age_acc_difference), value = mean(value))
significant <- correlations_top %>% group_by(omics,feature) %>% summarise(cor = cor(dif,value), p = pval_permutation(dif,value))
significant$sign <- sign(significant$cor)
top_selected <- significant %>% group_by(omics,sign) %>% summarise(feature = feature[which.min(p)]) %>% pull(feature) %>% as.character()
correlations_top <- correlations_top %>% filter(feature%in%top_selected)
significant$direction <- ifelse(significant$sign==1,"Positive","Negative")
